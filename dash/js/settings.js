/**
 * Drives-capital - SYSTEM METRICS SECURITY CONTROL CENTER (FRONTEND DESCRIPTOR WORKER)
 */
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchAssignedSecurityDefaults() {
        ...
        try {
            const connection = await fetch("https://api-v2-neon-alpha.vercel.app/api/bank/settings", {
                method: "GET",
                headers: {
    ...
    if (updatedPasswordFormNode) {
        updatedPasswordFormNode.addEventListener("submit", async (e) => {
            ...
            try {
                const response = await fetch("https://api-v2-neon-alpha.vercel.app/api/bank/settings", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${sessionTokenSignature}`,
