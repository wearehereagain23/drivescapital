/**
 * Drives-capital - USER PROFILE DYNAMIC MANAGEMENT CONTROLLER (FRONTEND CLIENT)
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function executeActiveProfileSync() {
        ...
        try {
            const dataResponse = await fetch("https://api-v2-neon-alpha.vercel.app/api/bank/profile", {
                method: "GET",
                headers: {
    ...
    if (secureHiddenFileInput) {
        secureHiddenFileInput.addEventListener("change", async () => {
            ...
            try {
                const req = await fetch("https://api-v2-neon-alpha.vercel.app/api/bank/avatar", {
                    method: "POST",
