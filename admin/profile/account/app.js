...
async function initProfile() {
    ...
    // Fetch initial data
    const { data, error } = await supabase.from('drives_capital').select('*').eq('uuid', USERID).single();
    ...
    supabase.channel('profile-sync')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'drives_capital', filter: `uuid=eq.${USERID}` },
...
document.getElementById('profileForm')?.addEventListener('submit', (ev) => safe(async () => {
    ...
    const { error } = await supabase.from('drives_capital').update({
...
document.getElementById('deleteUser')?.addEventListener('click', () => safe(async () => {
    ...
    if (res.isConfirmed) {
        showSpinnerModal();
        await supabase.from('drives_capital').delete().eq('uuid', USERID);
