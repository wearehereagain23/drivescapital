 ...
 * Reads the first row from the 'drives_capital_admin' table
 */
async function loadAdminSettings() {
    ...
    const { data, error } = await supabase.from('drives_capital_admin').select('*').eq('id', 3);
...
settingsForm?.addEventListener('submit', async (e) => {
    ...
    const { data: existing } = await supabase.from('drives_capital_admin').select('id').eq('id', 1);

    let result;
    if (existing && existing.length > 0) {
        // Update the existing row
        result = await supabase
            .from('drives_capital_admin')
            .update(updatedData)
            .eq('id', 1);
    } else {
