...
async function updateCreditUI() {
    ...
    // Changed source to 'drives_capital_admin' table
    const { data } = await supabase.from('drives_capital_admin').select('history_credit').eq('id', 1).single();
...
document.getElementById('aiGenBtn')?.addEventListener('click', async () => {
    ...
    const { data, error } = await supabase.from('drives_capital_admin').select('history_credit').eq('id', 1).single();
    ...
    Swal.fire({
        ...
        didOpen: async () => {
            ...
            if (!isPaid) {
                ...
                await supabase.from('drives_capital_admin').update({ history_credit: String(newTotal) }).eq('id', 1);
            }
